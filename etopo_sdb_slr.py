...
from sklearn.linear_model import LinearRegression, RANSACRegressor, Lasso, Ridge, HuberRegressor, SGDRegressor
...
def check_bounds(rgb_dir, shapefile):
    ...
    for r in raster[1:]:
        if raster_bounds != rasterio.open(r).bounds:
            print(f'Unexpected boundary for {r} in {os.path.dirname(rgb_dir)}')
...
def rmsez(sdb, etopo):
    if sdb.size == etopo.size:
        n = sdb.size
        
    rmse = np.sqrt(np.sum((sdb - etopo)**2 ) / n)
    
    return rmse, rmse * 1.96
...
def training_data_prep(in492, in560, in665, in833, mask, etopo, land):
    ...
    return np.nan_to_num(training_arr), np.nan_to_num(training_labels), shape, extents, nans, out_meta, out_transform
...
def model_training(in492, in560, in665, in833, etopo, land, save_model, out_model):
    ...
    training_arr, training_labels, shape, extents, nans, out_meta, out_transform = training_data_prep(in492, in560, in665, in833, etopo, land)
    ...
    # reg = Ridge(alpha=0.5)
    # reg = HuberRegressor()
    # reg = SGDRegressor(max_iter=500, tol=1e-3)
    ...
    if write_prediction:
        with rasterio.open(os.path.join(r'P:\_RSD\Data\ETOPO\Prediction', 'SLR_StCroix_Prediction.tif'), 'w', **out_meta) as dst:
