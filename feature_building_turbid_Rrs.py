    ...
    def feature_options(self, blue, green, red, red_704, red_740, nir_783, 
        ...
        for band in rasters:
            try:
                ...
                #                                           'masked_nechad_' + os.path.basename(band)[-6:-4] + '.tif')
        ...
        if ndti:
            ndti_aoi = Features.ODI_2(self, red_aoi, green_aoi, odi2_name='ndti.tif') # red_edge_704_aoi
            self.composite_features_list.append(ndti_aoi)
            features_list.append("'NDTI',")            
        if pSDBg:
...
def rgb_composite(rgb_dir): 
    ...
    for input_band in os.listdir(rgb_dir):
        try:
            if input_band.endswith('.tif') and 'Rrs_492' in input_band:
                print(f'Found blue band: {input_band}')
                rgb.append(os.path.join(rgb_dir, input_band))
            elif input_band.endswith('.tif') and 'Rrs_560' in input_band or 'Rrs_559' in input_band:
                print(f'Found green band: {input_band}')
                rgb.append(os.path.join(rgb_dir, input_band))
            elif input_band.endswith('.tif') and 'Rrs_665' in input_band:
                print(f'Found red band: {input_band}')
                rgb.append(os.path.join(rgb_dir, input_band))
            # elif input_band.endswith('.tif') and 'Rrs_833' in input_band:
            #     print(f'Found nir band: {input_band}')
            #     nirInput =  os.path.join(rgb_dir, input_band)
        except:
            print('May be issue with input file name. Expected ".tif" and "Rrs_(wavelength)" in name.')
...
def main():    
    ...
    num_options = sum(ft_options.values()) - (ft_options['window_size']) # exclude window size and include two for roughness
    if ft_options['pSDBg_roughness']:
        num_options += 1
    if ft_options['pSDBr_roughness']:
        num_options += 1
    if ft_options['nechad']:
        num_options += 1
    if ft_options['cmyk']:
        num_options += 1
